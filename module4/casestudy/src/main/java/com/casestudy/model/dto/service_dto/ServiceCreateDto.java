...
import com.casestudy.utils.CheckServiceId;
import org.springframework.validation.Errors;
import org.springframework.validation.Validator;

import javax.validation.constraints.Pattern;

public class ServiceCreateDto implements Validator {
    @Pattern(regexp = "^DV-[0-9]{4}$", message = "Mã dịch vụ sai định dạng")
    @CheckServiceId(message = "Mã dịch vụ đã tồn tại")
    ...
    public ServiceCreateDto(@Pattern(regexp = "^DV-[0-9]{4}$", message = "Mã dịch vụ ") String serviceId,
                            String serviceName,
                            String serviceCost,
                            String serviceArea,
    ...
    public void validate(Object target, Errors errors) {
        ServiceCreateDto serviceCreateDto = (ServiceCreateDto) target;
        if (serviceCreateDto.getServiceName().equals("Villa")){
            try {
                double checkDouble = Double.parseDouble(serviceCreateDto.poolArea);
                if (checkDouble < 0){
        ...
        if (serviceCreateDto.getServiceName().equals("Villa") || serviceCreateDto.getServiceName().equals("House")){
            try {
                double checkDouble = Double.parseDouble(serviceCreateDto.floor);
                if (checkDouble < 0){
            ...
            if (serviceCreateDto.description.isEmpty()){
                errors.rejectValue("description", "description.invalidFormat");
            }

            if (serviceCreateDto.standardRoom.isEmpty()){
